...
@torch.no_grad()
def evaluate_refiner(pipe: DepthRefinementPipeline, dataloader: DataLoader, device="cuda") -> float:
    total_loss = 0.0
    count = 0

    pbar = tqdm(dataloader)
    for rgb, depth_gt, path_str in pbar:
        rgb = rgb.to(device)
        ...
        np.save(f"data/experiments/ours/train_predictions/sample_{path_str[0][24:30]}_depth", d_refined.cpu().numpy())

        d = d_refined
        d_np = d.squeeze().cpu().numpy()  # (H, W)
        d_norm = (d_np - d_np.min()) / (d_np.max() - d_np.min() + 1e-6)

        # Apply 'plasma' colormap
        colormap = cm.get_cmap("plasma")
        d_colored = colormap(d_norm)[:, :, :3]  # Drop alpha channel, shape (H, W, 3)

        # Convert to uint8 for saving
        d_uint8 = (d_colored * 255).astype(np.uint8)

        # Save as image
        Image.fromarray(d_uint8).save(f"data/experiments/ours/train_predictions/sample_{path_str[0][24:30]}_depth.png")

        loss = si_log_loss(d_refined, depth_gt)
        total_loss += loss.item()
        count += 1

        # Update live mean loss in tqdm bar
        mean_loss = total_loss / count
        pbar.set_postfix({"Mean SI-Log RMSE": f"{mean_loss:.4f}"})

    print(f"✓ Evaluation complete — Mean SI-Log RMSE: {mean_loss:.4f}")
    return mean_loss

from torch.utils.data import Subset

train_ds = RGBDepthPairs(root="data/train/train", list_txt="data/train_list.txt")
selected_indices = [0, 26, 33, 71, 100]
subset_ds = Subset(train_ds, selected_indices)

train_loader = DataLoader(subset_ds, batch_size=1, shuffle=True,  num_workers=4, pin_memory=True)

pipe = DepthRefinementPipeline().to("cuda")

# Load checkpoint
load_refiner_for_eval(pipe, Path("<checkpoint_path>"))  # Replace with your checkpoint path

# Run evaluation
evaluate_refiner(pipe, train_loader)

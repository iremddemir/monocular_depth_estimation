...
def postprocess_depth(depth: np.ndarray, rgb: np.ndarray = None, uncertainty: np.ndarray = None) -> np.ndarray:
    ...
    # Normalize uncertainty if present
    def normalize(x):
        x = x - np.min(x)
        return x / (np.max(x) + 1e-6)

    # Uncertainty fusion (softer)
    if uncertainty is not None:
        unc = normalize(uncertainty)
        weight = np.exp(-unc * 4.0)  # softer fusion
        depth = weight * depth + (1 - weight) * cv2.medianBlur(depth, 5)

    # Median filter
    depth = cv2.medianBlur(depth.astype(np.float32), ksize=5)

    # Bilateral filter (optional: comment out if too soft)
    ...
    if rgb is not None:
        try:
            ...
            depth = xip.guidedFilter(guide=rgb_uint8, src=depth.astype(np.float32), radius=9, eps=1e-2)
        except ImportError:
            print("Skipping guided filtering (cv2.ximgproc missing).")
    """
    return depth
